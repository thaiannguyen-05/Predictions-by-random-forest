...
export class AuthController {
	...
	@ApiOperation({
		summary: 'User login',
		description: 'Authenticate user with email and password, returns JWT tokens in cookies'
	})
	...
	const result = await this.authService.oauth2Login(req.user as unknown as FacebookOAuth2User, res);
	const accessToken = result.tokens.accessToken;
	return res.redirect(`http://localhost:3000/auth/success?token=${accessToken}`);
	}
